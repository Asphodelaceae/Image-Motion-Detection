# -*- coding: utf-8 -*-
"""Homework_8_test.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1gEeo2PKHLpPXj_t6nPGBMlZ9E1c9n4nt


### SOME CODE AND TESTS PROVIDED BY CSE30 COURSE STAFF
...
* Second, download the notebook in .ipynb format (File > Download .ipynb) and upload the .ipynb file to [this form](https://docs.google.com/forms/d/e/1FAIpQLSdbOqZ_KZNoG7H8KRwYACtBBIZA178h6zry7ytHPYz-s7kKhA/viewform?usp=sf_link).  This homework is due on **Thursday, February 03, at 11:59pm**.
...
class SlidingWindowAveragerator(object):
    ...
    def avg(self):
      ...
      for x in self.vals:
          self.run_sum += x
      return(self.run_sum / (len(self.vals)))
    ...
    def std(self):
        ...
        run_sum = 0
        for x in self.vals:
          run_sum += (abs(x - self.avg))**2
        return np.sqrt(run_sum / len(self.vals))
...
"""Indeed, this approach would be able to detect most of the large outliers. We can use this idea to define a cleaned version of the data: when a reading is further away than two standard deviations from the average, we replace the reading.  Let us define a _CleanData_ class that does it for us.
...
class CleanData(object):
    ...
    def __init__(self, discount_factor):
        ...
        """
        # YOUR CODE HERE
        self.averagerator = DiscountedAveragerator(discount_factor)
    ...
    def filter(self, x, num_stdevs=2.):
        ...
        """
        # YOUR CODE HERE
        self.averagerator.add(x)
        cur_avg = self.averagerator.avg
        if x - cur_avg > (self.averagerator.std * num_stdevs):
          return cur_avg
        else:
          return x
...
class MotionDetection(object):
    ...
    def __init__(self, num_sigmas=4., discount=0.96):
        ...
        """
        # YOUR CODE HERE
        self.averagerator = DiscountedAveragerator(discount)
        self.harm = num_sigmas
    ...
    def detect_motion(self, img):
        ...
        # YOUR CODE HERE
        self.averagerator.add(img)
        disc_avg = self.averagerator.avg
        disc_std = self.averagerator.std
        left_bound = img < (disc_avg - (self.harm * disc_std))
        right_bound = img > (disc_avg + (self.harm * disc_std))
        moved_pixels = np.logical_or(left_bound, right_bound)
        return(np.max(moved_pixels, axis=2))
